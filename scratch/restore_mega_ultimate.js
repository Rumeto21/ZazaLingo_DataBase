...
const HEX_MAP = [
    ...
    { hex: 'c383c285c2ac', replace: 'ş' },     // Double mangled ş (Ã…ÂŸ)
    { hex: 'c383c283c382c287', replace: 'Ç' }, // Triple Ç
    { hex: 'c383c286c292c383c282c2a7', replace: 'ç' }, // Weird triple variant
    { hex: 'c383c286c292c383c282c2ae', replace: 'î' }, // Another triple î
    { hex: 'c3afc2bfc2bd', replace: 'u' },     // Replacement char mangled (Ã¯Â¿Â½) -> u
    { hex: 'efbfbd', replace: 'u' }            // Direct replacement char
];
...
function restoreBinary(source, targetTS, targetJSON) {
    ...
    // String level fixes for remaining cases
    ...
    if (startMatch) {
        ...
        try {
            ...
            console.error(`❌ JSON parse error for ${targetJSON}. Extraction might be too simple.`);
        }
    }
}

console.log('🚀 Starting ULTIMATE RECONSTRUCTION...');
...
console.log('🏁 Ultimate Reconstruction complete.');
